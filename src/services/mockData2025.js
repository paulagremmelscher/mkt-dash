...
export function getMockData2025() {
  ...
  // ROAS 2025 real (Métricas ventas R43): Ene:56.5, Feb:119.9, Mar:59.1, Abr:70.6, May:43.1, Jun:28.2
  const roasSeries = series([57,120,59,71,43,28,52,42,61,44,37,46])
  ...
  // CPL 2025 calculado: inversión / leads totales por mes
  const CPL_2025 = M.map((_,i) => null)  // se calcula abajo
  const convSeries = M.map((mes,i) => {
    const l = totalLeads[i].valor
    const v = ventas[i].valor
    const inv = inversionMensual[i].valor
  ...
  return {
    ...
    ventasUnicasB2B, ventasUnicasB2C, roasSeries,
