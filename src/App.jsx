...
const MAP_MONTHS={'Ene/26':'Ene/25','Feb/26':'Feb/25','Mar/26':'Mar/25','Abr/26':'Abr/25',
  'May/26':'May/25','Jun/26':'Jun/25','Jul/26':'Jul/25','Ago/26':'Ago/25'}

// ─────────────────────────────────────────────────────────────────
export default function App() {
  const {data,loading,error}=useDashboardData()
  const [selMonth,setSelMonth]=useState('Ago/26')
    ...
    <div style={{minHeight:'100vh',background:C.bg,fontFamily:'Roboto,sans-serif'}}>
      ...
      <main style={{maxWidth:1280,margin:'0 auto',padding:'22px 24px',display:'flex',flexDirection:'column',gap:24}}>
        ...
        <Section title="Leads y Canales de Contacto" icon={Users} open={false}>
          ...
          {/* Resumen anual: total de leads, CPA y CPL (mensual + acumulado) */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:14,marginTop:14}}>
            <CC title="Total de leads del año" sub="Suma acumulada Ene–actual">
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline'}}>
                  <span style={{fontSize:10,color:C.grayL,textTransform:'uppercase',letterSpacing:'0.12em'}}>2026 (YTD)</span>
                  <span style={{fontSize:'1.8rem',fontWeight:300,color:C.greenD}}>{formatNumber(d2026.kpis?.ytdLeads||0)}</span>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline'}}>
                  <span style={{fontSize:10,color:C.grayL,textTransform:'uppercase',letterSpacing:'0.12em'}}>2025 (total anual)</span>
                  <span style={{fontSize:'1.3rem',fontWeight:300,color:C.grayL}}>{formatNumber((d2025.totalLeads||[]).reduce((s,d)=>s+d.valor,0))}</span>
                </div>
              </div>
            </CC>
            <CC title="CPA — Costo por conversión" sub="Mensual (barras) y acumulado del año (línea)">
              <ResponsiveContainer width="100%" height={170}>
                <ComposedChart data={d2026.convSeries||[]} margin={{top:4,right:4,left:-16,bottom:0}}>
                  <CartesianGrid strokeDasharray="3 3" stroke={C.border} vertical={false}/>
                  <XAxis dataKey="mes" tick={{fontSize:10,fill:C.grayL}} axisLine={false} tickLine={false}/>
                  <YAxis tick={{fontSize:10,fill:C.grayL}} axisLine={false} tickLine={false} tickFormatter={v=>`$${v.toFixed(0)}`}/>
                  <Tooltip content={<RT prefix="USD "/>}/>
                  <Legend wrapperStyle={{paddingTop:6,fontSize:11}}/>
                  <Bar dataKey="cpa" name="CPA mensual" fill={CH.v2} radius={[2,2,0,0]} barSize={14}/>
                  <Line type="monotone" dataKey="cpaCum" name="CPA acumulado" stroke={CH.v1} strokeWidth={2.5} dot={{r:3,fill:CH.v1,strokeWidth:0}}/>
                </ComposedChart>
              </ResponsiveContainer>
            </CC>
            <CC title="CPL — Costo por lead" sub="Mensual (barras) y acumulado del año (línea)">
              <ResponsiveContainer width="100%" height={170}>
                <ComposedChart data={d2026.convSeries||[]} margin={{top:4,right:4,left:-16,bottom:0}}>
                  <CartesianGrid strokeDasharray="3 3" stroke={C.border} vertical={false}/>
                  <XAxis dataKey="mes" tick={{fontSize:10,fill:C.grayL}} axisLine={false} tickLine={false}/>
                  ...
                  <Tooltip content={<RT prefix="USD "/>}/>
                  <Legend wrapperStyle={{paddingTop:6,fontSize:11}}/>
                  <Bar dataKey="cpl" name="CPL mensual" fill={CH.g1} radius={[2,2,0,0]} barSize={14}/>
                  <Line type="monotone" dataKey="cplCum" name="CPL acumulado" stroke={CH.v1} strokeWidth={2.5} dot={{r:3,fill:CH.v1,strokeWidth:0}}/>
                </ComposedChart>
              </ResponsiveContainer>
            </CC>
          </div>
        ...
        <Section title="Ventas — Análisis Principal" icon={ShoppingBag} open={true}>
          ...
          {/* Ventas únicas por mes: 2026 vs 2025 */}
          <CC title="Ventas únicas por mes — 2026 vs 2025" sub="Comparativo mensual de unidades vendidas" style={{marginTop:14}}>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={yearSeries(d2026.ventas, d2025.ventas)} margin={{top:4,right:4,left:-10,bottom:0}} barGap={3} barCategoryGap="28%">
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} vertical={false}/>
                <XAxis dataKey="mes" tick={{fontSize:10,fill:C.grayL}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fontSize:10,fill:C.grayL}} axisLine={false} tickLine={false}/>
                <Tooltip content={<RT/>}/>
                <Legend wrapperStyle={{paddingTop:8,fontSize:11}}/>
                <Bar dataKey="2025" name="Ventas 2025" fill={CH.g1} radius={[3,3,0,0]} barSize={16}/>
                <Bar dataKey="2026" name="Ventas 2026" fill={CH.v1} radius={[3,3,0,0]} barSize={16}/>
              </BarChart>
            </ResponsiveContainer>
          </CC>
